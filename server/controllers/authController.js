...
exports.register = async (req, res) => {
    try {
        ...
        if (adminCode !== secrets.ADMIN_REGISTRATION_CODE) return res.status(403).json({ message: "Invalid Admin Registration Code." });
        if (!password || password.length < 6) return res.status(400).json({ message: "Password must be at least 6 characters long." });
        if (await db.User.findOne({ where: { email } })) return res.status(400).json({ message: "Admin with this email already exists." });
